(() => {
  ...
  const _cfg   = (script && script.dataset && script.dataset.client) ? script.dataset : (window.WG_CONFIG || {});

  const CLIENT_ID       = _cfg.client        || '';
  const WEBHOOK_URL     = '/api/chat';
  const GREETING        = _cfg.greeting      || 'Hi! How can I help you today?';
  const AGENT_NAME      = _cfg.name          || 'Alex';
  const SUBTITLE        = _cfg.subtitle      || 'AI Assistant';
  const COLOR_PRIMARY   = _cfg.colorPrimary  || '#1D6FFF';
  const COLOR_DARK      = _cfg.colorDark     || '#1558D6';
  ...
  const fontsAlreadyLoaded = [...document.styleSheets].some(ss => {
    try { return ss.href && ss.href.includes('fonts.googleapis.com'); } catch { return false; }
  });
  ...
  style.textContent = `
    ...
    .wg-call-bar:hover { background: rgba(6,15,34,0.9); }
